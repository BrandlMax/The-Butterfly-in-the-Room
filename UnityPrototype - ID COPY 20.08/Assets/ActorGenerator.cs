...
public class ActorGenerator : MonoBehaviour
{
    ...
    public void addActor(string[] data)
    {
        ...
        // Set texture
        /*
        Texture2D t = LoadPNG("Assets/ImageFiles/" + actor.name + ".png");
        if(t != null)
        {
        ...
        */
        ...
        // Sprite s = LoadSprite("Sprites/" + actor.name);

        
        string fileName = actor.name;

        if(actor.name.Contains("/") || actor.name.Contains("&"))
        {
            Debug.Log(actor.name + " contains -");
            fileName = actor.name.Replace("/", "");
            fileName = fileName.Replace("&", "");
            Debug.Log("Replacing with " + fileName);
        }
        

        Sprite s = Resources.Load<Sprite>("Sprites/" + fileName);

        if (s != null)
        {
            visualization.GetComponent<SphereController>().image.sprite = s;
        } else
        {
            Debug.Log("S: " + s);
        }
    ...
    {        
        Sprite s = null;        
        s = Resources.Load(filePath) as Sprite;
